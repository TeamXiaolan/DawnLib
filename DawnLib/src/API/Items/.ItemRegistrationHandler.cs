...
static class ItemRegistrationHandler
{
    ...
    internal static void UpdateItemWeightsOnLevel(SelectableLevel level)
    {
        ...
        foreach (DawnItemInfo itemInfo in LethalContent.Items.Values)
        {
            ...
            SpawnWeightContext ctx = new(level.GetDawnInfo(), RoundManager.Instance.dungeonGenerator?.Generator?.DungeonFlow?.GetDawnInfo(), TimeOfDayRefs.GetCurrentWeatherEffect(level)?.GetDawnInfo() ?? (LethalContent.Weathers.IsFrozen ? LethalContent.Weathers[WeatherKeys.None] : null));
